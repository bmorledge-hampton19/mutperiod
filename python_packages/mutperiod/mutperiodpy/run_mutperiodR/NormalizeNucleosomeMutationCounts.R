...
if (length(args) == 3) {
  normalizedCounts = normalizeNucleosomeMutationCounts(args[1],args[2],args[3])
} else if (length(args) == 4) {
  normalizedCounts = normalizeNucleosomeMutationCounts(args[1],args[2],args[3], as.numeric(args[4]))
} else if (length(args) == 0) {
  normalizedCounts = selectInputAndRun()
} else {
