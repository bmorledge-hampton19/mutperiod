...
- Create another directory beneath this newly created directory named "?????????????".
- Obtain a fasta file for the hg19 genome assembly.  This can be obtained from the USCS Genome Browser [here](https://hgdownload.soe.ucsc.edu/goldenPath/hg19/bigZips/).
- Unzip the genome fasta file, ensure that it is named "hg19.fa", and move it to the hg19 directory.
- Obtain the nucleosome positioning file generated by the [BBGLab](<https://bbglab.irbbarcelona.org/>) from [here](https://notAValidLinkYet.com).
- Move the nucleosome positioning file into the ???????? directory.
